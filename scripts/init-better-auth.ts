...
async function initBetterAuth() {
	console.info('🔧 Initializing Better Auth database tables...');
    ...
    `;
		...
		console.info('📋 Existing database tables:');
		console.info(tables);
		...
		console.info(`🔐 Accounts in database: ${accountCount}`);

		// Check if we have sessions
		const sessionCount = await prisma.session.count();
		console.info(`📱 Sessions in database: ${sessionCount}`);

		console.info('✅ Better Auth database check completed!');
	} catch (error) {

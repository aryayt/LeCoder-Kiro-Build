...
export class PipelineErrorHandler {
	...
	static createError(
		...
		return {
			type,
			message,
			code: options.code || PipelineErrorHandler.generateErrorCode(type),
			stage: options.stage,
			stageName: options.stageName,
			retryable: options.retryable ?? PipelineErrorHandler.isRetryableByDefault(type),
	...
	private static generateErrorCode(type: PipelineErrorType): string {
		...
		const [prefix = 'pipeline'] = type.split('_');
		const typeCode = prefix.toLowerCase();
	...
	static parseError(
		...
		if (error instanceof Error) {
			...
			if (message.includes('rate limit') || message.includes('quota')) {
				return PipelineErrorHandler.createError(PipelineErrorType.RATE_LIMIT_ERROR, error.message, {
					...context,
					retryable: false,
				});
			}
			...
			if (message.includes('timeout') || message.includes('timed out')) {
				return PipelineErrorHandler.createError(PipelineErrorType.TIMEOUT_ERROR, error.message, {
					...context,
					retryable: true,
				});
			}
			...
			if (message.includes('api key') || message.includes('authentication')) {
				return PipelineErrorHandler.createError(
			...
			if (message.includes('database') || message.includes('prisma')) {
				return PipelineErrorHandler.createError(PipelineErrorType.DATABASE_ERROR, error.message, {
					...context,
					retryable: true,
				});
			}
	...
	static formatErrorForLogging(error: PipelineError): string {
		const parts = [`[${error.type}]`, `Code: ${error.code}`, `Message: ${error.message}`];
		...
		if (error.stage) {
			parts.push(`Stage: ${error.stage}${error.stageName ? ` (${error.stageName})` : ''}`);
		}
	...
	static getSuggestedRetryDelay(error: PipelineError, attempt: number): number {
		...
		switch (error.type) {
			...
			case PipelineErrorType.AI_SERVICE_ERROR:
				return baseDelay * 2 ** attempt; // Exponential backoff

			case PipelineErrorType.DATABASE_ERROR:
				return baseDelay * (attempt + 1); // Linear backoff

			case PipelineErrorType.TIMEOUT_ERROR:
				return baseDelay * 1.5 ** attempt; // Moderate exponential backoff

			default:
				return baseDelay * 2 ** attempt; // Default exponential backoff
		}
	}
	...
	static getRecoverySuggestions(error: PipelineError): string[] {
		...
		switch (error.type) {
			...
			case PipelineErrorType.CONFIGURATION_ERROR:
				suggestions.push('Check your API keys in the environment configuration');
				suggestions.push('Verify that all required services are properly configured');
				break;

			case PipelineErrorType.AI_SERVICE_ERROR:
				suggestions.push('Check the status of the AI service provider');
				suggestions.push('Try switching to a different AI provider if available');
				break;
			...
			case PipelineErrorType.TIMEOUT_ERROR:
				...
				suggestions.push('Consider breaking down large documents into smaller sections');
				break;
		...
		if (error.retryable) {
			suggestions.unshift('This error is retryable - the system will automatically retry');
		}
	...
	static logError(error: PipelineError, logger: Console = console): void {
		...
		if (PipelineErrorHandler.shouldFailImmediately(error)) {

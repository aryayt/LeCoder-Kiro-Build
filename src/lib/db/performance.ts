...
import { ProjectStatus, type StageStatus } from '@prisma/client';
...
export class DatabasePerformanceManager {
	...
	async getSystemStats(): Promise<{
		...
		return cache.getOrSet(
			cacheKey,
			async () => {
				const [totalProjects, activeProjects, completedProjects, totalUsers, recentActivity] =
					await Promise.all([
						db.project.count(),
	 ...
	 */
	async cleanupOldData(daysOld = 30): Promise<{

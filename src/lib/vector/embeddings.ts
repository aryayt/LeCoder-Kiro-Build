 ...
 */
/**
 * Hugging Face EmbeddingGemma API integration
 */
...
export async function generateEmbeddings(
	chunks: TextChunk[],
	provider: 'openai' | 'google' | 'huggingface' = 'huggingface',
	userId?: string,
	customApiKey?: string
	...
	try {
		...
		if (customApiKey) {
			apiKey = customApiKey;
		} else if (userId && (provider === 'openai' || provider === 'google')) {
			// Only use API key service for supported providers
		...
		if (provider === 'openai') {
			...
			const client = apiKey ? createGoogleGenerativeAI({ apiKey: key }) : google;
...
class SimpleVectorStore {
	...
	async search(
		...
		return findSimilarChunks(queryText, chunks, topK, provider, userId, customApiKey);
	}
}
...
export async function processPdfForVectorStorage(
	...
	try {
		...
		// Store embeddings
		await vectorStore.set(projectId, embeddedChunks);
